...
import {
  ...
  getReplayPlaygroundResponse,
  getReplayModelsResponse,
...
/** Canned demo response for a POST endpoint, or null if none exists. */
function postDemoResponse(endpoint: string, body: Record<string, unknown>): NextResponse | null {
  ...
  if (endpoint === "/api/playground/generate") {
    return NextResponse.json(
      getReplayPlaygroundResponse({
        prompt: (body.prompt as string) || "",
        max_new_tokens: (body.max_new_tokens as number) || 16,
        solver_budget: (body.solver_budget as number) || 6,
      })
    );
  }
  return null;
...
function getDemoResponse(endpoint: string): NextResponse | null {
  ...
  if (endpoint === "/api/auction/traces") {
    return NextResponse.json(getReplayTracesListResponse());
  }
  if (endpoint.startsWith("/api/auction/traces/")) {
    const seed = Number(endpoint.slice("/api/auction/traces/".length));
    return NextResponse.json(getReplayTracesResponse(Number.isFinite(seed) ? seed : 42));
  }
  return null;
}
...
export async function POST(
  ...
  if (denied) {
    // Non-admin visitors get a clearly-flagged demo replay instead of an error.
    const demo = postDemoResponse(endpoint, body);
    if (demo) return demo;
    return denied;
  }
