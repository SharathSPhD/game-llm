"""EXP07: Contraction Check for EqLM-v2 (Spectral Norm + Damped Residuals).

Compares three arms over 300 training steps on real BabyLM stream:
  A1: EqLM-v1 (spectral_norm=False) — non-contractive baseline
  A2: EqLM-v2 (spectral_norm=True + damping) — contraction-enforced
  A3: ExplicitLM reference — parameter-matched baseline

Primary metrics:
  - Final loss (A2 final ≤ A1 final + 5% per preregistration)
  - Solver convergence rate (A2 > 80% per preregistration)
  - Mean solver iterations (A2 < A1 per preregistration)
  - Wall time (A2 expected ~2-3x A3 due to fixed-point solving)
...
from kinetic_ai.data import BabyLMDataLoader, build_token_stream, load_babylm_dataset, load_or_build_tokenizer
from kinetic_ai.models.eqlm import EqLM, EqLMConfig, ExplicitLM, count_params, match_explicit_width
...
def plot_loss_curves(arms_data: dict, output_dir: Path) -> None:
    ...
    colors = {"A1": "#E69F00", "A2": "#56B4E9", "A3": "#009E73"}
...
def main():
    ...
    # ========== Aggregate Results ==========
    arms_data = {"A1": a1_result, "A2": a2_result, "A3": a3_result}
    ...
    print(f"  [1] A2 final ≤ A3 final + 5%: {v2_final:.4f} <= {v1_final * 1.05:.4f} = {criterion1}")

    if "solver_convergence_rate" in a2_result:
        criterion2 = a2_result["solver_convergence_rate"] > 0.80
        print(f"  [2] A2 convergence > 80%: {a2_result['solver_convergence_rate']:.1%} = {criterion2}")
    else:
        criterion2 = None
    ...
    results = {
        ...
        "arms": {
            ...
            "A2": {
                "name": "EqLM-v2 (spectral_norm=True, damping=0.2)",
                "params": a2_params,
            ...
            "A3": {
                "name": "EqLM-v1 (spectral_norm=False)",
                "params": a3_params,
