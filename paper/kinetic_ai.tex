...
\subsection{EqLM Pretraining on BabyLM: Smoke Pipeline Validation (F9)}
...
\noindent
BLiMP accuracies ($0.459$–$0.537$) are noise around chance at these loss scales; they validate the tokenization→train→eval pipeline, not LM capability. A3's non-learning is a confound: raw Euclidean MMD lacks adaptive preconditioning needed for small-LR initialization. ADR 0003 schedules MagneticAdamW implementation to decouple magnetism from optimizer adaptivity for H3 testing. (Smoke status, ADR 0003 optimizer decision, method finding F9, config sha 1398dc8f, commit 39284dd5).
...
\begin{itemize}
\item Smoke-scale LM results (BabyLM 10M tokens, BLiMP 1000 pairs): BLiMP accuracies ($0.459$–$0.537$) are noise around chance; H1's full benchmark requires Tier B scaling (GB10 full training).
...
\item BLiMP full runs pending: current smoke shows tokenization→train→eval pipeline works; full-run model selection requires completion of H3 (MagneticAdamW) and revisit of weight initialization (ADR 0003).
\end{itemize}
