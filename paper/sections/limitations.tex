...
rather than the method. GSM8K is excluded from Table~\ref{tab:ladder} for a
related reason: strict-match scored zero for all four models, which measures
whether an instruction-tuned model emits a particular answer convention rather
than whether it can carry out the arithmetic, and no generative claim is made
from a harness configuration exhibiting that failure.
